...
describe("Markuplint Integration", () => {
  ...
  <img src="b">
    ...
    const requiredAttrCount = violations.filter(v => v.ruleId === "required-attr").length;
    expect(requiredAttrCount).toBe(2);
    
    // require-accessible-name reports for 'alt' (1 violation).
    const accessibleNameCount = violations.filter(v => v.ruleId === "require-accessible-name").length;
